  ...
  persist(
    (set) => ({
      ...
      clickDifference: (paintingName: string, differenceId: string) => set((state) => {
        ...
        if (Object.keys(state.paintings[paintingName].differenceIds).length === paintingsLibrary[paintingName].totalDiffs) {
      ...
      resetPainting: (paintingName: string) => set((state) => {
        state.paintings[paintingName] = {
          differenceIds: {},
          timeSpent_seconds: 0,
          isComplete: false
