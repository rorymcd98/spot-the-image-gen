...
import PaintingPanel, {
    PaintingPanelProps,
    PaintingPosition,
} from './components/PaintingPanel/PaintingPanel';
...
const Home: React.FC = () => {
    ...
    const zoomPainting = (e: React.WheelEvent<HTMLSpanElement>): void => {
        ...
        newPaintingPosition.zoomRatio = Math.min(
            Math.max(nextZoomRatio, minZoomRatio),
            maxZoomRatio
        );
    ...
    function createFollowMouseOnPainting(panelId: string) {
        return (e: React.MouseEvent<HTMLSpanElement, MouseEvent>) => {
            ...
            const paintingPanel = document.getElementById(
                panelId + '-painting'
            );
            ...
            newPaintingPosition.xFraction =
                (e.clientX - rect.left) / targetWidth;
            newPaintingPosition.yFraction =
                (e.clientY - rect.top) / targetHeight;
    ...
    const xNormalized = constrain(
        0.5 + xRange * (paintingPosition.xFraction - 0.5)
    );
    const yNormalized = constrain(
        0.5 + yRange * (paintingPosition.yFraction - 0.5)
    );

    //Translate the position of the mouse to a tranasform value for the painting (linear interpolation)
    const xTransform =
        100 * ((0.5 - xNormalized) * (1 - 1 / paintingPosition.zoomRatio));
    const yTransform =
        100 * ((0.5 - yNormalized) * (1 - 1 / paintingPosition.zoomRatio));
    ...
    const isVertical = calulateIsVertical(
        paintingAspectRatio,
        windowAspectRatio
    );
